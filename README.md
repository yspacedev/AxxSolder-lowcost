...
Changes to be made:
Add jumper in place of mosfet and ideal diode controller for usage with USB-C power only
Replace high side mosfet driver with discrete components for a cheap bootstrapping driver
Use LCSC for passives (and change footprints if absolutely necessary)
Use lower cost uC?? (probably not since $4 extra is worth not having to port the code (and it allows any build AxxAxx makes to be compatible on the low cost version and any changes I make to be compatible on the original hardware)
